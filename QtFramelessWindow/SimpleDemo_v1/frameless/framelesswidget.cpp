    ...
    : QWidget(parent), d(new FramelessWidgetPrivate)
{
    ...
    auto makeTitleBar = [this] {
        ...
        layout->setContentsMargins(QMargins(11, 0, 0, 0));
        layout->setSpacing(0);
...
bool FramelessWidget::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
    ...
    case WM_NCHITTEST:
    {
        ...
        auto borderY = GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(92);//SM_CXPADDEDBORDER
        ...
        auto margins = d->layout->contentsMargins();
        localPos.rx() -= margins.left();
        localPos.ry() -= margins.top();
